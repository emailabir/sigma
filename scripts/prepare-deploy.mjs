...
if(!['github','email','firebase'].includes(mode))throw new Error('SIGMA_AUTH_MODE must be github, email or firebase.');
const names=['ALPACA_API_KEY_ID','ALPACA_API_SECRET_KEY',...(mode==='firebase'?[]:mode==='email'?['SIGMA_BREVO_API_KEY','SIGMA_EMAIL_FROM']:['SIGMA_GITHUB_CLIENT_ID','SIGMA_GITHUB_CLIENT_SECRET'])];
if(mode==='email'){
...
if(mode==='firebase'){
 const project=process.env.SIGMA_FIREBASE_PROJECT_ID,key=process.env.SIGMA_FIREBASE_API_KEY;
 if(!/^[a-z][a-z0-9-]{4,28}[a-z0-9]$/.test(project??'')||!/^[A-Za-z0-9_-]{20,100}$/.test(key??''))throw new Error('Set SIGMA_FIREBASE_PROJECT_ID and SIGMA_FIREBASE_API_KEY repository variables before deploying Firebase login.');
 config.vars.SIGMA_FIREBASE_PROJECT_ID=project;
 config.vars.SIGMA_FIREBASE_API_KEY=key;
}
